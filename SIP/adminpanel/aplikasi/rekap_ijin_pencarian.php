...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<div class="judulisikanan">
			...
			<div class="menuhorisontalaktif-ujung"><a href="rekap_ijin.php">Rekap Ijin</a></div>
		...
		<div class="atastabel" style="margin:5px">
			...
			<tr><td class="isiankanan" width="250px">Pilih Kelas </td><td class="isian">
				<select name="kelas">
					<option value="<?php echo $_POST['kelas']; ?>" selected> <?php echo $_POST['kelas']; ?> </option>
					<?php
			...
			<tr><td class="isiankanan" width="250px">No Induk Siswa</td><td class="isian"><input type="text" class="pencarian" name="noinduk" id="noinduk" value="<?php echo $_POST['noinduk']; ?>" ></td></tr>
			<tr><td class="isiankanan" width="250px"> </td><td class="isian"> <input type="submit" class="batal" value="Tampil Data"> </td></tr>
			<td class="isiankanan" width="250px"><a href='../adminpanel/aplikasi/cetak/print_pencarian_ijin_data.php?noinduk=<?php echo $_POST['noinduk'] ?>&kelas=<?php echo $_POST['kelas'] ?>' target='_blank'>Tampilan Cetak </a>
		...
		<table class="tabel" id="results">
			...
			<?php
				$no=1;
				$absen=mysql_query("SELECT DISTINCT siswa.siswa_id as id , siswa.siswa_name as nama , result_proc.n_induk , result_proc.nisn , result_proc.kelas_name , result_proc.tahun_ajaran , result_proc.semester  ,
									 (SELECT COUNT(ijin_k.siswa_id)
									  FROM ijin_k,siswa
									  WHERE (siswa.siswa_id = ijin_k.siswa_id AND siswa_name = nama)) AS TOTAL_IZIN
									 FROM result_proc , siswa where result_proc.siswa_name = siswa.siswa_name AND result_proc.kelas_name = '$_POST[kelas]' AND result_proc.n_induk = '$_POST[noinduk]' ");
				...
				<td class="tabel"><div id="button"><a href="?nid=<?php echo $row['id'];?>&#popup">Detail</a></div></td>
			</tr>
			<?php $no++; } 
				} else { 
			?>
			<tr class="tabel"><td class="tabel" colspan="9"><font color="red"><b>DATA RINCIAN IZIN ABSENSI SISWA BELUM TERSEDIA / DATA KELAS & NO INDUK TIDAK SESUAI</b></font></td></tr>

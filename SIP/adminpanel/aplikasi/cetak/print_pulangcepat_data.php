...
echo " <div class=\"sd_left\">
		...
		$absen = mysql_query("SELECT DISTINCT siswa.siswa_name as nama, result_proc.n_induk , result_proc.nisn , result_proc.kelas_name , result_proc.tahun_ajaran , result_proc.semester ,
									  ...
									  FROM result_proc , siswa WHERE result_proc.pulang_cepat AND result_proc.siswa_name = siswa.siswa_name AND result_proc.kelas_name = '$_GET[kelas]' AND result_proc.n_induk = '$_GET[noinduk]'");
		...
		// explode di gunakan untuk memisahkan data per kata
			...
			<td class=tabel align=center width=20%>$row[TOTAL_PULANG_CEPAT]</td>
			</tr>";
			} // End Of While
			echo "</table>
			    </div>

...
<form method="POST" class="form-horizontal text-left" action="<?php $_SERVER['PHP_SELF'];?>">
       ...
       <div class="table-responsive">
		...
		<?php
				...
				$absen=mysql_query("SELECT hari,scan_date,result_proc.n_induk,tahun_ajaran,result_proc.siswa_name,kelas_name,jadwal_name,jam_masuk,scan_in,terlambat,jam_pulang,scan_out,pulang_cepat,status_hadir 
									FROM result_proc , siswa where result_proc.scan_date between '".InggrisTgl($tanggal_awal)."' and '".InggrisTgl($tanggal_akhir)."' and result_proc.siswa_name = siswa.siswa_name AND result_proc.siswa_id = '$ot[id_siswa]' ORDER BY `result_proc`.`scan_date` DESC");

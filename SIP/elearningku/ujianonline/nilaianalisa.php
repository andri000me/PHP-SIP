...
<img src=\"images/images_admin/icon_admin_user.png\" align=\"absmiddle\" class=\"img_title\" /> Soal Analisa
</div>";
?>
<?php
include "set/koneksi.php";
       ...
       if(isset($_POST['submit'])){
			...
			for ($i=0;$i<$jumlah;$i++){
				...
				if (empty($pilihan[$nomor])){
					...
					$query=mysql_query("select * from tsoal where soalid='$nomor' and jawaban='$jawaban'");
		...
		?>
        <form action="simpananalisa.php" method="post" id="form-area">
		
		<?php 
		if($kosong==0){
		echo " Terimakasih Anda Sudah Menjawab Semua Soal Silahkan Simpan Soal";
		}else{ echo "Anda Belum Menjawab $kosong Soal <a href=\"javascript:history.back()\"> <blink><strong>silahkan isi kembali soal</strong></blink></a> atau Simpan";
		} 
		?>

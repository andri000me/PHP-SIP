<?php
if($_GET[id]==1){
	$mading="Mading Karya Seni";
}
elseif($_GET[id]==2){
	$mading="Mading Karya Tulis";
}
?>


<div id="ms" class="panel1 panel-primary">
<div class="panel-heading"><h3 style="font-family:Bree Serif;" class="panel-title"><?php echo $mading; ?></h3></div>
...
<?php	$batas= 5;
		...
		$mading =mysql_query("SELECT * FROM sh_mading, sh_mading_kategori, sh_siswa WHERE sh_mading.id_kategori=sh_mading_kategori.id_mading_kategori AND sh_mading.nama_siswa=sh_siswa.nama_siswa AND status_terbit='1' AND id_kategori='$_GET[id]' ORDER BY sh_mading.id_mading DESC LIMIT $posisi, $batas");
		$cek_mading=mysql_num_rows($mading);

		if($cek_mading > 0){
		while ($r=mysql_fetch_array($mading)){
		$hitung_komen=mysql_query("SELECT * FROM sh_komentar_mading WHERE id_mading='$r[id_mading]' AND status_terima='1'");
...
?>		
		...
		<a style="margin-left:10px;" href="?p=detprofil&id=<?php echo $r[nama_siswa];?>#popup"> <?php echo $r[nama_siswa];?></a><br/><p style="margin-left:60px;"><?php echo tgl_indo($r[tanggal_posting]);?><br/><small><?php echo "<a  style='margin-left:0px;' href='?p=katmading&id=$r[id_mading_kategori]'>$r[nama_mading_kategori]</a>";?></small></p>
		</div>
...
elseif ($_SESSION[orangtua])
	{if($jml_cek==0){
		...
		if ($halaman > 1){
		$prev=$halaman-1;
		echo 	"	<div class='hal' style='float: left'><a href='?p=madingKaryaSeni&halaman=$prev' title='Halaman Sebelumnya'>&laquo; Sebelumnya</a></div>";
		}
		if ($halaman < $jmlhal) {
		$next=$halaman+1;
		echo "	<div class='hal' style='float: right'><a href='?p=madingKaryaSeni&halaman=$next' title='halaman Berikutnya'>Berikutnya &raquo;</a></div>"; }
}
else { 
?>

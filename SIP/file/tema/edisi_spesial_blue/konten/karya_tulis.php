...
<div id="mt" class="panel1 panel-primary">
<div class="panel-heading"><h3 style="font-family:Bree Serif;" class="panel-title">Mading Karya Tulis</h3></div>
...
echo "</div>";
	...
	if ($_SESSION['siswa']){
		...
		echo "<p style=padding-right:10px;' class='text-right'>
				<a href='?p=".MD5('like')."&to=like&no=$r[id_mading]&hal=madingKaryaTulis' class='btn btn-default btn-sm'>
				  <span class='glyphicon glyphicon-thumbs-up'></span> Like
				</a></p>";}
		else{
			echo "<p style=padding-right:10px;' class='text-right'>
				<a href='?p=".MD5('like')."&to=unlike&no=$r[id_mading]&hal=madingKaryaTulis' class='btn btn-info btn-sm'>
...
elseif ($_SESSION['orangtua'])
	{if($jml_cek==0){
		...
		if ($halaman > 1){
		$prev=$halaman-1;
		echo 	"	<div class='hal' style='float: left'><a href='?p=madingKaryaTulis&halaman=$prev' title='Halaman Sebelumnya'>&laquo; Sebelumnya</a></div>";
		}
		if ($halaman < $jmlhal) {
		$next=$halaman+1;
		echo "	<div class='hal' style='float: right'><a href='?p=madingKaryaTulis&halaman=$next' title='halaman Berikutnya'>Berikutnya &raquo;</a></div>"; }
}
else { 
?>
